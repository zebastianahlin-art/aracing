...
$signalCoverageNotes = is_array($payload['signal_coverage_notes'] ?? null) ? $payload['signal_coverage_notes'] : [];
?>
<section class="card">
  <div class="topline">
    <h1 style="margin:0;">AI Demand Signals / Interest Without Conversion (v1)</h1>
    <small>Beslutsstöd: inga automatiska pris-, produkt- eller merch-ändringar görs.</small>
  </div>

  <form method="get" action="/admin/ai-demand-signals" class="grid" style="margin-top:12px;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;">
    <div>
    ...
    <div>
      ...
      <a class="btn" href="/admin/ai-demand-signals">Rensa</a>
    </div>
  </form>

  <p>
    Totalt insights: <strong><?= (int) ($counts['total'] ?? 0) ?></strong> |
    High interest / low conversion: <strong><?= (int) ($counts['high_interest_low_conversion'] ?? 0) ?></strong> |
    Repeated interest / no stock: <strong><?= (int) ($counts['repeated_interest_no_stock'] ?? 0) ?></strong> |
    Compare interest: <strong><?= (int) ($counts['compare_interest_signal'] ?? 0) ?></strong> |
    Viewed not bought: <strong><?= (int) ($counts['viewed_not_bought'] ?? 0) ?></strong>
  </p>
</section>
...
<section class="card" style="margin-top:12px;">
  <div class="topline">
    ...
    <small>Dynamisk beräkning on-demand, ingen tung analytics- eller trackingplattform i v1.</small>
  </div>

  <?php if ($rows === []): ?>
    <p>Inga demand insights matchade aktuella filter.</p>
  <?php else: ?>
    <table class="table compact">
      ...
      <?php foreach ($rows as $row): ?>
        <tr>
          ...
          <td>
            <span class="pill <?= in_array((string) ($row['insight_type'] ?? ''), ['repeated_interest_no_stock'], true) ? 'bad' : 'warn' ?>">
              <?= htmlspecialchars((string) ($row['insight_label'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
            </span>
          </td>
          <td>
            Wishlist: <strong><?= (int) ($row['wishlist_count'] ?? 0) ?></strong><br>
            Compare: <strong><?= (int) ($row['compare_count'] ?? 0) ?></strong><br>
            Aktiva stock alerts: <strong><?= (int) ($row['active_stock_alerts'] ?? 0) ?></strong><br>
            Recent interest signal: <strong><?= (int) ($row['recent_view_signal'] ?? 0) ?></strong><br>
            Sålt 30/60 dagar: <strong><?= (int) ($row['sold_last_30_days'] ?? 0) ?> / <?= (int) ($row['sold_last_60_days'] ?? 0) ?></strong><br>
            Lagerstatus: <strong><?= htmlspecialchars((string) ($row['stock_status'] ?? '-'), ENT_QUOTES, 'UTF-8') ?></strong> (<?= (int) ($row['stock_quantity'] ?? 0) ?>)
          </td>
          <td>
...
<section class="card" style="margin-top:12px;">
  <h2 style="margin-top:0;">Signal-täckning i v1</h2>
  <ul>
    <?php foreach ($signalCoverageNotes as $signal => $note): ?>
      <li><strong><?= htmlspecialchars((string) $signal, ENT_QUOTES, 'UTF-8') ?>:</strong> <?= htmlspecialchars((string) $note, ENT_QUOTES, 'UTF-8') ?></li>
    <?php endforeach; ?>
  </ul>
</section>
<?php
$content = (string) ob_get_clean();
$title = 'AI Demand Signals | Admin';
